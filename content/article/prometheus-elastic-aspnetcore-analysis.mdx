...
- **Content**: Raw data, inverted indices, cluster configuration
...
- **Backup Storage**: ~75GB
- **Content**: Compressed metrics, configuration files
- **Retention**: Standard time-series retention

**Key Insight**: Both systems use the same backup infrastructure and processes. The cost difference is purely storage-based, not operational complexity.


### Operational Costs Breakdown Explanation
...
- **Elastic Stack**: €1,200/year (team has existing expertise)
  - Elasticsearch version upgrades with rolling updates
  - APM Server compatibility testing
  - Plugin updates and compatibility verification
- **Prometheus Stack**: €1,200/year (learning curve for new technology)
  - Prometheus server updates (requires learning new procedures)
  - AlertManager configuration updates (new syntax and concepts)
  - Exporter updates and compatibility testing (unfamiliar ecosystem)
  - Additional time needed for troubleshooting during transition period

**Dashboard Development:**
- **Elastic Stack**: €3,600 one-time (€1,200/year amortized)
  - Custom ASP.NET Core dashboard creation
  - Business metrics visualization setup
  - Alert dashboard configuration
- **Prometheus Stack**: €800 one-time (€267/year amortized)
  - Minimal customization of existing Microsoft dashboards
  - Basic alert configuration

**Important Note**: These operational costs are **identical between Windows and Linux** because the same maintenance activities, monitoring requirements, and administrative tasks are needed regardless of the operating system. The upgrade costs reflect the reality that switching to Prometheus requires a learning curve for the team, even though the technology itself may be simpler to maintain once mastered.
...
*Note: All cost estimations involving labor (Integration, Dashboard Development, Management Overhead) are based on an internal senior developer rate of €95/hour.*
...
## Appendix: Linux Deployment Alternative
...
In 2025, most organizations deploy monitoring infrastructure on Linux because:
